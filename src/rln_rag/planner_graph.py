...
def generate_plan(
    ...
    plan_dict = _apply_domain_templates(question, plan_dict)
    plan_dict = _ensure_home_sequence(plan_dict)
    return plan_dict, docs


def _apply_domain_templates(question: str, plan: dict) -> dict:
    """Force high-priority templates (e.g., 교수님 미션) regardless of LLM output."""
    ...
    if any(keyword in question for keyword in PROFESSOR_KEYWORDS):
        if not any(
            step.get("action") == "navigate"
            and (step.get("params") or {}).get("target") == "professor_office"
            for step in steps
        ):
            steps.insert(0, {"action": "navigate", "params": {"target": "professor_office"}})

    plan["plan"] = [
        step for step in steps if step.get("action") != "talk_to_person"
    ]
    return plan


def _ensure_home_sequence(plan: dict) -> dict:
    """Make sure summarize_mission follows a basecamp navigate step."""

    steps = plan.get("plan")
    if not isinstance(steps, list):
        return plan

    def has_home_before(index: int) -> bool:
        for step in steps[:index]:
            if (
                step.get("action") == "navigate"
                and (step.get("params") or {}).get("target") == "basecamp"
            ):
                return True
        return False

    for idx, step in enumerate(list(steps)):
        if step.get("action") == "summarize_mission" and not has_home_before(idx):
            steps.insert(idx, {"action": "navigate", "params": {"target": "basecamp"}})
            break

    if not any(step.get("action") == "summarize_mission" for step in steps):
        steps.append({"action": "navigate", "params": {"target": "basecamp"}})
        steps.append({"action": "summarize_mission", "params": {}})
